...
func readTextFile(path string) (string, error) {
	...
	if err!=nil { return "",err }
	return cleanScript(string(decoded)),nil
}
...
func readDocx(path string) (string,error) {
	...
	return "",fmt.Errorf("DOCX 中没有找到正文")
}

func readLegacyDoc(path string)(string,error){
	if err:=ole.CoInitializeEx(0,ole.COINIT_APARTMENTTHREADED);err!=nil && err!=ole.S_FALSE{
		return "",err
	}
	defer ole.CoUninitialize()

	unknown,err:=oleutil.CreateObject("Word.Application")
	if err!=nil{return "",fmt.Errorf("读取 DOC 需要本机 Microsoft Word：%v",err)}
	...
	docVar,err:=oleutil.CallMethod(docs,"Open",path,false,true)
	if err!=nil{return "",fmt.Errorf("打开 DOC 失败：%v",err)}
	doc:=docVar.ToIDispatch()
	defer doc.Release()
	defer oleutil.CallMethod(doc,"Close",false)
	defer oleutil.CallMethod(word,"Quit")

	contentVar,err:=oleutil.GetProperty(doc,"Content")
	if err!=nil{return "",err}
	content:=contentVar.ToIDispatch()
	defer content.Release()

	textVar,err:=oleutil.GetProperty(content,"Text")
	if err!=nil{return "",err}
	return cleanScript(textVar.ToString()),nil
}
...
func splitScript(s string,maxRunes int)[]string{
	...
	for _,r:=range []rune(s){
		cur=append(cur,r)
		n:=len(cur)
		if strings.ContainsRune(hard,r){flush();continue}
		if n>=maxRunes && strings.ContainsRune(soft,r){flush();continue}
		if n>=maxRunes+6{
			cut:=-1
...
func rebalanceShort(in []string,max int)[]string{
	...
	for i:=1;i<len(out);i++{
		if len([]rune(out[i]))<=4{
			prev:=[]rune(out[i-1]);cur:=[]rune(out[i])
